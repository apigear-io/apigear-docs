...
The wrapped object `IHello& impl` is just the object that fulfills the `IHello` interface, so the implementation can be e.g. the [`OlinkClient`](/template-cpp14/docs/features/olink.md) version of `Hello`.

The `📜hello.tracer.h` and `📜hello.tracer.cpp` files contain helper to prepare interface specific trace data for the general tracing [TraceClient](/template-cpp14/docs/features/monitor.md#tracing-example)
